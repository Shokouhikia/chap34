...
export default function OpsPackingPage() {
  ...
  async function load() {
    ...
    try {
      const [tpl, pending] = await Promise.all([
        opsApi.packingTemplate(),
        opsApi.packingPending(),
      ]);
      setItems(tpl.items);
  ...
  async function toggleItem(o: PackOrder, key: string, value: boolean) {
    // Optimistic local update, then persist the single toggle.
    setOrders((prev) =>
      prev.map((x) =>
        x.id === o.id
    ...
    try {
      if (o.fulfillment_status === "ready_to_pack") {
        await opsApi.packingStart(o.id);
      }
      await opsApi.packingChecklist(o.id, { [key]: value });
    } catch (err) {
      setError(err instanceof Error ? err.message : "خطا");
    }
  }
  ...
  async function confirm(o: PackOrder) {
    setError(null);
    try {
      await opsApi.packingConfirm(o.id);
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "خطا");
    }

...
export default function OpsShipmentsPage() {
  ...
  async function createShipment() {
    setError(null);
    try {
      const res = await opsApi.createShipment(Array.from(selected));
  ...
  async function openShipment(id: string) {
    ...
    try {
      const data = await opsApi.getShipment(id);
  ...
  async function act(fn: () => Promise<unknown>, thenReopen?: string) {
    ...
    try {
      ...
      await load();
      if (thenReopen) {
        const data = await opsApi.getShipment(thenReopen);
        setDetailOrders(data.orders);
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "خطا");
          ...
          <div key={s.id} className="card">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-4">
                ...
                {s.handed_to_post_at && (
                  <span className="rounded-full bg-green-50 px-2 py-1 text-xs font-bold text-success">
                    تحویل پست شده
                  </span>
                )}
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() =>
                    opsApi.postDeliveryList(s.id).catch((e) => setError(e.message))
                  }
                ...
                <button
                  onClick={() =>
                    opsApi.shipmentLabels(s.id).catch((e) => setError(e.message))
                  }
                  className="rounded-lg border border-line px-3 py-1.5 text-xs font-bold text-navy"
                >
                  برچسب‌های پستی
                ...
                {!s.handed_to_post_at && (
                  <button
                    onClick={() => act(() => opsApi.handToPost(s.id), s.id)}
                ...
                {detailOrders.map((o) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => act(() => opsApi.markDelivered(o.id), s.id)}
                        className="rounded-lg bg-success px-2.5 py-1.5 text-xs font-bold text-white"
                      >
                        تحویل مشتری
                      </button>
                      <button
                        onClick={() =>
                          opsApi.orderLabel(o.id).catch((e) => setError(e.message))
                        }

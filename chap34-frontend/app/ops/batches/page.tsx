...
export default function OpsBatchesPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      setBatches(await opsApi.listBatches());
    } catch (err) {
        ...
        {batches.map((b) => (
          <div key={b.id} className="card">
            ...
            <div className="flex flex-wrap gap-2">
              {b.status === "queued" && (
                <button
                  onClick={() => act(() => opsApi.startPrinting(b.id))}
              ...
              {b.status === "printing" && (
                <button
                  onClick={() => act(() => opsApi.markPrinted(b.id))}
                  className="rounded-lg bg-navy px-3 py-1.5 text-xs font-bold text-white"
                >
                  اتمام چاپ
                </button>
              ...
              <button
                onClick={() =>
                  opsApi.batchSheets(b.id, "pdf").catch((e) => setError(e.message))
                }
